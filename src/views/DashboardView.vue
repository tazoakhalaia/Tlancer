    ...
    <div class="bg-gray-800 min-h-screen">
        ...
        <div class="m-auto w-1/2 bg-gray-200 mt-2 pb-2 rounded-md">
            ...
            <div class="w-full mt-2 p-2 rounded-md">
                <textarea v-model="titleValue"  class="resize-none outline-none rounded-sm w-full h-6" placeholder="Write Your Title"></textarea>
                <textarea v-model="contentValue" class="resize-none outline-none rounded-sm w-full h-20" placeholder="Write Your Content"></textarea>
                </div>
        ...
        <div v-for="(news, index) in newsArray" :key="index" class="w-1/2 bg-gray-200 p-2 m-auto mt-4 break-all rounded-md shadow-[rgba(0,_0,_0,_0.25)_0px_25px_50px_-12px] pb-4">
            ...
            <h1 v-if="!isEditing(news.id)" class="text-md mt-2 text-black">{{ news.content }}</h1>
            ...
            <div class="mt-2 w-full">
                <input v-model="commentValue[index]" @change="writeComment(news.id, index)" class="w-1/2 rounded-md pl-2 bg-gray-500 h-10 outline-none" type="text" placeholder="Write Comment">
            </div>
            <hr class="h-0.5 bg-black mt-2">
            <div v-for="(comment, index) in news.comment" :key="index" class="mt-2 p-2 w-full flex justify-between">
                <div class="w-1/2 break-all">
                <h1 v-if="!isEditingComment(comment.id)" class="text-black"> ->  {{ comment.content }}</h1>
...
async function logOut(){
    let response = await axios.post('http://exam.tlancer.net/api/logout', config)
    localStorage.clear()
    router.push({ path: '/' })
    console.log(response);
}
