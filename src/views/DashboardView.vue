<template>
  <div class="relative bg-gray-800 min-h-screen">
    <div>
      <uploadSuccess :successMessage="successOrFail" />
    </div>
    <header class="bg-gray-800 h-20 flex justify-center items-center">
      <h1 class="text-white font-bold">Post new article</h1>
      <div>
        <button @click="logOut" class="ml-4 bg-gray-300 rounded-sm p-2">Log Out</button>
      </div>
    </header>
    <div class="m-auto w-1/2 bg-gray-200 mt-2 p-2 rounded-md">
      ...
      <div class="w-full mt-2 p-2 rounded-md">
        <textarea
          v-model="titleValue"
          class="resize-none outline-none rounded-sm w-full h-6 placeholder:text-black pl-2"
          placeholder="Write Your Title"
        ></textarea>
        <textarea
          v-model="contentValue"
          class="resize-none outline-none rounded-sm w-full h-20 placeholder:text-black pl-2"
          placeholder="Write Your Content"
        ></textarea>
      </div>
      <h1 class="text-red-500 font-bold">{{ articleAddMessage }}</h1>
      <Button @click.prevent="sendArticleRequest" buttonName="Post" />
    </div>
    <div
      v-for="(news, index) in newsArray"
      :key="index"
      class="w-1/2 bg-gray-200 p-2 m-auto mt-4 break-all rounded-md shadow-[rgba(0,_0,_0,_0.25)_0px_25px_50px_-12px] pb-4"
    >
      <h1 class="font-bold text-md text-center text-black underline">{{ news.title }}</h1>
      <h1 v-if="!isEditing(news.id)" class="text-md font-bold mt-2 text-black">
        {{ news.content }}
      </h1>
      <input
        class="mb-2 w-full rounded-sm pl-2 h-8"
        v-else
        type="text"
        v-model="editedContetnValue[index]"
        placeholder="Write Your Content"
      />
      <div v-if="news.user_id === loginUserId">
        <hr class="h-0.5 bg-black mt-2 mb-2" />
        <button @click="deleteNews(news.id)" class="p-1 bg-red-500 rounded-sm text-white">
          Delete
        </button>
        <button
          v-if="isEditing(news.id)"
          @click="updateContent(news.id, news.title, index)"
          class="p-1 bg-orange-500 ml-2 rounded-sm text-white"
        >
          Save
        </button>
        <button
          v-else
          @click="editId = news.id"
          class="p-1 bg-orange-500 ml-2 rounded-sm text-white"
        >
          Edit
        </button>
      ...
      <div class="mt-2 w-full">
        <input
          ...
          @change="writeComment(news.id, index)"
          class="w-1/2 rounded-md pl-2 bg-gray-500 h-10 outline-none placeholder:text-white text-white"
          type="text"
      ...
      <hr class="h-0.5 bg-black mt-2" />
      <div
        v-for="(comment, index) in news.comment"
        :key="index"
        class="mt-2 p-2 w-full flex justify-between"
      >
        <div class="w-1/2 break-all flex items-center">
          <div>
            <CommentIcon class="w-5 h-5 mr-2" />
          </div>
          <h1 v-if="!isEditingComment(comment.id)" class="text-black text-sm font-medium">
            {{ comment.content }}
          </h1>
          <input v-else type="text" v-model="editedCommentValue[index]" />
        </div>
        <div v-if="comment.user_id === loginUserId">
          <button @click="deletComment(comment.id)" class="bg-red-500 p-1 text-white rounded-sm">
            Delete
          </button>
          <button
            v-if="isEditingComment(comment.id)"
            @click="updateComment(news.id, comment.id, index)"
            class="p-1 bg-orange-500 ml-2 rounded-sm text-white"
          >
            Save
          </button>
          <button
            ...
            class="bg-orange-500 p-1 text-white rounded-sm ml-2"
          >
            Edit
          </button>
        </div>
...
import uploadSuccess from '../components/shared/uploadSuccess.vue'
import { useRouter } from 'vue-router'
let token = localStorage.getItem('token')
let successOrFail = ref('')
let articleAddMessage = ref('')
let titleValue = ref('')
...
async function sendArticleRequest() {
  if (titleValue.value === '' || contentValue.value === '') {
    articleAddMessage.value = 'Please fill all fields'
    setTimeout(() => {
      articleAddMessage.value = ''
    }, 2000)
  }

  try {
    let response = await axios.post(
      'http://exam.tlancer.net/api/news',
      {
    ...
    if (response.status === 200) {
      successOrFail.value = 'News Uploaded Successfully refresh the page to see your news'
      setTimeout(() => {
        successOrFail.value = ''
      }, 2000)
    }
    ...
    if (error.response && error.response.status != 200) {
      successOrFail.value = 'Please fill all fields'
      setTimeout(() => {
        successOrFail.value = ''
      }, 2000)
    }
...
async function updateContent(id, title, index) {
  ...
  let response = await axios.put(
    `http://exam.tlancer.net/api/news/${id}`,
    {
      title: title,
...
async function writeComment(newsId, index) {
  ...
  let response = await axios.post(
    'http://exam.tlancer.net/api/comments',
    {
...
async function updateComment(id, commentId, index) {
  let response = await axios.put(
    `http://exam.tlancer.net/api/comments/${commentId}`,
    {
...
async function logOut() {
  localStorage.clear()
  router.push({ path: '/' })
  await axios.post('http://exam.tlancer.net/api/logout', null, config)
}
</script>
